...
| `CAPSULE_REPO` | overrides the profile's `path` for `capsule-provision` and `capsule <slot> fetch`. It still wins: what the lookup replaced is the *baked default*, not the environment. Nothing supplies it: the module's wrapper did until SL-001 (`ISS-008`), and a default there was a second answer in front of the document's |
