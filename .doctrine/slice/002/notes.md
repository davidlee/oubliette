...
against code and the pinned nixpkgs. It found five faults (commit `075ead9`
lists them) and all five were revised through the run. Fault A, a failed clone
unmarking an earlier unscrubbed clone, is the kind a second pass exists to find:
...
2. **`agent` processes outside any logind session.** A `systemd --user` unit
   ...
   deleted or reseeded.
3. **Pre-existing state generally**, which is Fault A's class: a leftover marker
...
### Produced
- `design.md` sec-1..sec-8 — materialised from run `dr-01a0a2ae…`; all eight walked with the user, sec-3/4/5/7/8 revised for faults A–E (`075ead9`)
